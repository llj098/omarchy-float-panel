...
- `Super+Up/Down` keeps directional focus in Tiling mode; in Float mode it natively maximizes/restores the active window inside the gapped work area, retaining the bar and client chrome.
...
- `Super+Tab`/`Super+Shift+Tab` cycle and raise windows in Tiling mode; in Float mode they restore Omarchy's default next/previous-workspace behavior.
...
In a floating workspace, `Super+Left` and `Super+Right` snap the active window to the left or right half of the current monitor. Geometry is calculated from the monitor's scale, reserved bar area, live `gaps_out`/`gaps_in`, and border size, so snapped windows retain the same outer and middle margins as tiling and work with the bar on any edge. `Super+Up` sets Hyprland's native maximized state and `Super+Down` unsets it. The integration sets `general.float_gaps = -1`, which on supported Hyprland inherits `gaps_out`; centered keyboard resizing uses that inherited gap, border, reserved area, scale, and transform to clamp against the active window's own monitor. When Hyprland migrates a Float workspace to another monitor, normal mapped floating windows are synchronously shrunk only as needed and repositioned inside that monitor's usable floating work area; hidden windows, tiled workspaces, and native maximized/fullscreen reflow are left untouched. `Super+F` toggles split fullscreen state (`internal = 2`, `client = 0`) to fill the display while retaining client chrome. In a tiling workspace these keys retain Omarchy's original relative-resize, directional-focus, or synchronized-fullscreen behavior.
