...
- `Super+Up/Down` keeps directional focus in Tiling mode; in Float mode it adds/removes the active window from Hyprland's native Monocle subset, retaining the bar and client chrome while allowing multiple windows to stay work-area sized without resize-on-switch.
...
In a floating workspace, `Super+Left` and `Super+Right` snap the active window to the left or right half of the current monitor. Geometry is calculated from the monitor's scale, reserved bar area, live `gaps_out`/`gaps_in`, and border size, so snapped windows retain the same outer and middle margins as tiling and work with the bar on any edge. `Super+Up` saves the exact floating box in a live-window tag, changes only that window to tiled, and applies the built-in `monocle` tiled layout to the workspace. All such windows remain work-area sized; native Monocle focus handling selects the visible member without unmaximizing and resizing the previous one. `Super+Down` returns the selected member to floating, restores/clamps its saved box, and restores the workspace's captured prior tiled layout after the final member leaves. Config reloads reconstruct this subset from the window tags. Monocle is geometry/layout state, so clients intentionally report `fullscreen = 0` and `fullscreenClient = 0` rather than receiving a maximized state. The integration sets `general.float_gaps = -1`, which on supported Hyprland inherits `gaps_out`; centered keyboard resizing uses that inherited gap, border, reserved area, scale, and transform to clamp against the active window's own monitor. When Hyprland migrates a Float workspace to another monitor, normal mapped floating windows are synchronously shrunk only as needed and repositioned inside that monitor's usable floating work area; Monocle members reflow through the native layout. `Super+F` toggles split fullscreen state (`internal = 2`, `client = 0`) to fill the display while retaining client chrome. In a tiling workspace these keys retain Omarchy's original relative-resize, directional-focus, or synchronized-fullscreen behavior.
