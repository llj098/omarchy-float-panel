...
Hyprland should reload the configuration automatically. Automatic repair after same-monitor scale, mode, transform, reserved-area, or gap changes additionally requires rebuilding Hyprland 0.56.2 with `patches/hyprland-0.56.2-work-area-event.patch`. The patch emits one `workspace.work_area_changed` Lua event only after an actually changed floating work area has passed through native layout recalculation; the plugin then runs the same defensive fitter used by monitor migration and reload.

After updating an already loaded plugin checkout, let the file watcher settle and then restart Omarchy Shell once: the watcher notices QML/JavaScript changes, but the running engine can retain an imported JavaScript module cache. Do not overlap the restart with an in-progress file copy/hot reload.
