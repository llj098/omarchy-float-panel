...
- `Alt+Tab` opens a centered, launcher-themed MRU window list on every regular workspace, in both Tiling and Float modes. The first chord snapshots live `j/clients` data from Hyprland's request socket rather than trusting Quickshell's event cache, so a dropped event socket cannot leave ghost applications in the list. Every window is a separate item even when several belong to the same application; minimized windows are included. Tab/Shift+Tab uses Hyprland's native keyboard repeat to move only the highlight. Releasing either Alt key commits even when Shift remains held; if the live snapshot is still loading, the response is committed immediately instead of cancelling the switch.
- Floating mode is independent per workspace and persists across Hyprland config reloads and logins.
- Float windows without application-specified placement remember their last closed free, half-screen, or geometric-max placement per workspace and restore it when reopened.
- New parented windows without application-specified placement are initially centered over their concrete parent, then fitted to the monitor work area; application-specified placement remains application-owned.
- Application minimum-size hints are ignored globally without application-class rules; maximum-size hints retain Hyprland's native behavior.
- No daemon, polling loop, fixed workspace ID, or hyprbars plugin is used.

Hyprland supports tiled and floating windows concurrently. The Lua integration keeps only a set of workspace names whose normal windows should be floating. For each enabled workspace it enables a standard workspace-matched `float` window rule, so new windows are Float before layout placement; current and moved windows are reconciled separately.
...
`float-panel.lua` loads the bridge with Hyprland's native `hl.plugin.load()` API when the built `.so` exists. `hl.plugin.float_panel.window_semantics(address)` remains read-only and limited to parent/transient/override/window-type state, concrete parent address, and application-specified placement. Maximize requests use a separate raw `float_panel.maximize_request(window, bool)` custom event, which the bridge reattaches to Hyprland's rebuilt Lua event handler after every config reload. Lua owns all placement and persistence policy: it centers an unpositioned child over a resolvable parent and then runs the normal work-area fit. Application-specified placement remains application-owned. Missing or failing native reads degrade safely; persistence fails closed when semantics are unavailable.

Lua also tags standard auxiliary windows such as utility, tooltip, menu and popup windows; one tag-matched Hyprland rule disables their compositor border. This is application-independent and applies to existing and newly opened windows; ordinary windows retain their border. Client-owned transparent shadow margins are not cropped or resized.
...
TaskList clicks follow a deterministic per-application state ring. One window toggles `A → minimized → A`; multiple windows cycle in stable launch/address order and then minimize the whole group, for example `A → B → minimized → A`. Entering a group while another app is active first selects that application's live most-recent window; subsequent clicks continue the stable ring from that window. A minimized next window is restored before focus/raise, and the final-to-minimized transition moves every visible member to the workspace's dedicated minimized special workspace in one Hyprland callback. Alt-Tab is deliberately different: it does not group by application, and labels each independently selectable window with its application name and window title.
...
The model admits only toplevels whose Hyprland IPC object explicitly reports `mapped === true`; an associated toplevel with an absent or not-yet-populated IPC mapping is not enough. This excludes malformed auxiliary surfaces that reach Quickshell with an address but empty IPC identity/mapping fields. Malformed identities containing embedded NUL bytes are rejected as a secondary guard. Neither filter matches a window title or executable name.

Compatibility note: the live `j/clients` reads used by Alt-Tab and TaskList click selection are workarounds for Quickshell 0.3.1 not reconnecting its Hyprland event socket after disconnection ([Quickshell #989](https://github.com/quickshell-mirror/quickshell/issues/989)). Once a reconnecting Quickshell release is deployed and verified, both paths should return to the reactive Quickshell window model and remove the direct request-socket workaround.
...
Lua, TaskList, and AppSwitcher records all use the `[fatlj.float-panel]` prefix. A topology event writes one privacy-safe `reflow.window` record only for each window whose geometry actually changed; no-op checks write nothing. The record contains the trigger, source/target bounds, old/new geometry and requested resize/move. Formal logs may contain addresses, application class, and workspace/monitor identifiers, but never window titles, typed text, chat/content text, image contents, or other user content.
